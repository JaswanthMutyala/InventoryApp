...
async function POST(request) {
    try {
        ...
        // Validate admin role with password
        let userRole = 'member';
        ...
        if (role === 'admin') {
            ...
            if (!member_password) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: 'Member password is required'
                }, {
                    status: 400
                });
            }
        ...
        let user = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].prepare('SELECT * FROM users WHERE student_id = ?').get(student_id);
        if (!user) {
            // User doesn't exist - only admins can be auto-created
            if (role === 'admin') {
                const result = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].prepare('INSERT INTO users (student_id, name, role, password) VALUES (?, ?, ?, ?)').run(student_id, name, userRole, userPassword);
                user = {
                ...
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: 'User not found. Please ask admin to add you to the system.'
                }, {
                    status: 403
                });
            }
        } else {
            // User exists - verify password for members
            if (role === 'member' && user.password !== member_password) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ...
            // If logging in as admin, ensure stored role is admin
            if (userRole === 'admin' && user.role !== 'admin') {
                __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].prepare('UPDATE users SET role = ? WHERE id = ?').run('admin', user.id);
                user.role = 'admin';
            }
            // Update name if provided and different
            if (user.name !== name) {
                __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].prepare('UPDATE users SET name = ? WHERE id = ?').run(name, user.id);
                user.name = name;
            }
        }
        // Ensure response reflects effective role
        const responseUser = {
            id: user.id,
        ...
        // Log the access
        __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].prepare('INSERT INTO access_logs (user_id, student_id, name, role) VALUES (?, ?, ?, ?)').run(user.id, student_id, name, userRole);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1db077da._.js.map